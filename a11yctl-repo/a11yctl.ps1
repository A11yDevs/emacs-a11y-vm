...
if ($PSVersionTable.PSVersion.Major -lt 6 -and $PSCommandPath) {
    ...
    if (-not $hasBom) {
        ...
        Write-Host '[a11yctl] Arquivo atualizado para UTF-8. Execute o comando novamente.' -ForegroundColor Yellow
        exit 0
    }
}
...
$A11YCTL_FALLBACK_VERSION = '0.2.0'
$EA11CTL_FALLBACK_VERSION = $A11YCTL_FALLBACK_VERSION  # alias legado
$A11YCTL_OWNER = 'A11yDevs'
$EA11CTL_OWNER = $A11YCTL_OWNER  # alias legado
$A11YCTL_REPO = 'a11yctl'
$EA11CTL_REPO = $A11YCTL_REPO  # alias legado
$A11YCTL_BRANCH = 'main'
$EA11CTL_BRANCH = $A11YCTL_BRANCH  # alias legado
$A11YCTL_RELEASE_BASE_URL = 'https://argmap.inf.ufg.br/a11ydevs'
$EA11CTL_RELEASE_BASE_URL = $A11YCTL_RELEASE_BASE_URL  # alias legado
...
function Write-EA11Warn {
    param([string]$Message)
    Write-Host "[a11yctl] $Message" -ForegroundColor Yellow
}

function Write-EA11Error {
    param([string]$Message)
    Write-Host "[a11yctl] $Message" -ForegroundColor Red
}

function Show-Help {
    @"
a11yctl - CLI do projeto A11yDevs para gerenciamento de VMs, pacotes e ambientes acessiveis [HOST - WINDOWS]

Uso:
  a11yctl (abre modo interativo)
  a11yctl help|-h|--help
  a11yctl version|--version [-c|--check-update]
  a11yctl self-update|update [-f|--force]
  a11yctl migrate|migrate-state
...
Nota: Dentro da VM (guest context), execute: a11yctl share
Debug: use EA11_DEBUG=1 ou passe --debug em vm start
Compatibilidade: ea11ctl e um alias temporario de a11yctl
"@
}
...
function Get-RemoteCliVersion {
    $remoteVersionUrl = "https://raw.githubusercontent.com/$A11YCTL_OWNER/$A11YCTL_REPO/$A11YCTL_BRANCH/VERSION"
...
function Invoke-AccessibleDownload {
    ...
    try {
        ...
        while ($true) {
            ...
            if ($contentLength -gt 0) {
                ...
                if ($percent -ge $nextPercent) {
                    ...
                    Write-Host "[a11yctl] Progresso ${Label}: $percent% ($doneMb/$totalMb MB)"
            ...
            elseif ($downloadedBytes -ge $nextUnknownReportBytes) {
                ...
                Write-Host "[a11yctl] Progresso ${Label}: $doneMb MB baixados"
        ...
        if ($contentLength -gt 0) {
            ...
            Write-Host "[a11yctl] Progresso ${Label}: 100% ($doneMb/$totalMb MB)"
        }
        else {
            $doneMb = [math]::Round($downloadedBytes / 1MB, 1)
            Write-Progress -Id $progressId -Activity "Baixando $Label..." -Status 'Concluido' -Completed
            Write-Host "[a11yctl] Download concluido ($doneMb MB): $Label"
        }
...
function Download-FileWithFallback {
    ...
    $attempts = @(
        @{ Uri = "https://raw.githubusercontent.com/$Owner/$Repo/$Ref/$File?cb=$CacheBust"; Headers = (Get-GitHubRawHeaders); Label = 'raw+cb+headers' },
        @{ Uri = "https://raw.githubusercontent.com/$Owner/$Repo/$Ref/$File"; Headers = (Get-GitHubRawHeaders); Label = 'raw+headers' },
        @{ Uri = "https://raw.githubusercontent.com/$Owner/$Repo/$Ref/$File"; Headers = $null; Label = 'raw-sem-headers' },
        @{ Uri = "https://github.com/$Owner/$Repo/raw/$Ref/$File"; Headers = $null; Label = 'github-raw-fallback' }
    )
...
function Invoke-VersionCommand {
    ...
    Write-Host "a11yctl v$localVersion"
    ...
    try {
        ...
        if ($cmp -gt 0) {
            ...
            Write-Host 'Use: a11yctl self-update' -ForegroundColor Green
        }
        elseif ($cmp -eq 0) {
...
function Invoke-SelfUpdate {
    ...
    $files = @('a11yctl.ps1', 'a11yctl.cmd', 'ea11ctl.ps1', 'ea11ctl.cmd', 'VERSION')
...
function Get-EA11StateDirectory {
    $base = Join-Path (Get-HomeDirectoryPath) '.a11yctl'
...
function Invoke-QemuVMList {
    ...
    if (-not $files) {
        Write-EA11Info 'Nenhuma VM QEMU registrada em ~/.a11yctl.'
        return
    }
...
function Invoke-QemuVMStop {
    ...
    if (-not $state) {
        Write-EA11Warn "VM QEMU '$vmName' nao possui estado registrado em ~/.a11yctl."
        return
    }
    ...
    if ($state -and $state.pid) {
        ...
        if ($vmPid -gt 0 -and (Get-ProcessByIdSafe -ProcessId $vmPid)) {
            if ($force) {
                ...
                throw "VM '$vmName' esta em execucao. Use a11yctl vm stop --name $vmName ou --force."
            }
        }
    }
...
function Invoke-Migrate {
    ...
    $legacyDir = Join-Path (Get-HomeDirectoryPath) '.emacs-a11y-vm'
    $newDir = Get-EA11StateDirectory  # ~/.a11yctl

    if (-not (Test-Path $legacyDir)) {
        Write-EA11Info "Nenhuma instalacao antiga encontrada em $legacyDir."
        Write-EA11Info "Nada a migrar."
        return
    }

    Write-EA11Info "Instalacao antiga encontrada em: $legacyDir"
    Write-EA11Info "Diretorio de destino: $newDir"

    # Garantir que o diretorio destino existe
    if (-not (Test-Path $newDir)) {
        New-Item -ItemType Directory -Path $newDir -Force | Out-Null
    }

    # Migrar arquivos de disco de VM (qcow2, vmdk, vdi, vhd, vhdx, iso, ova)
    ...
    foreach ($disk in $diskFiles) {
        ...
        if (Test-Path $dest) {
            # Evitar sobrescrever: usar sufixo .migrated
            $base = [System.IO.Path]::GetFileNameWithoutExtension($dest)
            $ext  = [System.IO.Path]::GetExtension($dest)
            $dest = Join-Path $destDir ($base + '.migrated' + $ext)
            Write-EA11Warn "Arquivo ja existe, copiando como: $dest"
            $skippedCount++
        }
        Write-EA11Info "Copiando disco: $($disk.Name) -> $dest"
        Copy-Item -Path $disk.FullName -Destination $dest -Force
        $copiedCount++
    }

    # Migrar config.env (com substituicao de caminhos)
    $configExt = @('.env', '.json')
    ...
    foreach ($cfg in $configFiles) {
        ...
        if (-not (Test-Path $dest)) {
            ...
            # Substituir referências ao diretório legado pelos novos caminhos
            ...
            Write-EA11Info "Config migrada: $($cfg.Name) -> $dest"
        } else {
            Write-EA11Warn "Config ja existe, pulando: $dest"
        }
    }

    Write-Host ""
    Write-Host "=== Migracao concluida ===" -ForegroundColor Green
    Write-Host "Discos copiados: $copiedCount" -ForegroundColor Cyan
    if ($skippedCount -gt 0) {
        Write-Host "Conflitos renomeados (.migrated): $skippedCount" -ForegroundColor Yellow
    }
    Write-Host "Diretorio antigo preservado: $legacyDir" -ForegroundColor Cyan
    Write-Host "Diretorio novo: $newDir" -ForegroundColor Cyan
    Write-Host ""
    Write-EA11Info "ATENCAO: o diretorio antigo ($legacyDir) NAO foi removido."
    Write-EA11Info "Verifique o novo diretorio e remova o antigo manualmente quando estiver seguro."
}
...
function Invoke-Uninstall {
    ...
    if (-not $yes) {
        ...
        if ($purgeState) {
            Write-EA11Warn 'Tambem removera ~/.a11yctl (VMs, estado e logs).'
        }
        $reply = Read-Host 'Digite "yes" para confirmar'
        if ($reply -ne 'yes') {
    ...
    $installDir = $PSScriptRoot
    $toRemove = @('a11yctl.cmd', 'ea11ctl.cmd', 'VERSION')
    ...
    Write-Host 'a11yctl desinstalado neste diretorio.' -ForegroundColor Green
}
...
function Invoke-QemuVMStatus {
    ...
    $state = Load-QemuState -VMName $vmName
    if (-not $state) {
        Write-EA11Warn "VM QEMU '$vmName' nao registrada em ~/.a11yctl."
        return
    }
...
function Invoke-RootCommand {
    ...
    switch ($root) {
        ...
        'migrate' { Invoke-Migrate -Tokens $rest }
        'migrate-state' { Invoke-Migrate -Tokens $rest }
...
function Get-InteractivePrompt {
    ...
    switch ($Context) {
        'vm' { return 'a11yctl vm> ' }
        'vm_config' { return 'a11yctl vm config> ' }
        'vm_host_share' { return 'a11yctl vm host-share> ' }
        'host' { return 'a11yctl host> ' }
        default { return 'a11yctl> ' }
    }
}
